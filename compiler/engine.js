...
onmessage = (msg) => {
  switch (msg.data[0]) {
    case 'compile':
      let result = ''
      try {
        const as_array = msg.data[1].split('\n')
        ...
        const ast = JSON.stringify(token_dump, null, 2)
...
function find_operation (pattern, string) {
  const operators = all_matches(pattern, string)
...
function set_token (name, operation, exprs, line) {
  const add = { name: operation, type: 'expression', arguments: { expr1: exprs[0], expr2: exprs[1] } }
  const set_var = { name: 'set', type: 'command', arguments: { expr: add, name: name }, line: line }
  return set_var
}
...
function write_operands (expr1, expr2, type) {
  const result = []
  const [expr1_prefix, expr1_reg] = translate(expr1, type)
  const [expr2_prefix, expr2_reg] = translate(expr2, type)
...
function write_operand (expr, type) {
  let result = []
  const [expr_prefix, expr_reg] = translate(expr, type)
  result = expr_prefix
...
function operation_assignment_token (var_name, op, value_token) {
  ...
  const token = set_token(var_name, op, [var_token, value_token])
...
function alloc_block (size) {
  ...
  if (!state.scope.startsWith('sys.')) {
    const allocated_slots = 1023 - state.free_ram[state.scope].length
...
function alloc_global_block (size) {
  const old_state = state.scope
  state.scope = '[global]'
  const addrs = alloc_block(size)
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (tokens[i].type === 'expression' && tokens[i].name != 'function' || typeof tokens[i] === undefined) {
      ...
      if (tokens[i].name === 'function') {
        ...
        const function_type = state.function_table[tokens[i].arguments.name].data_type
...
function load_lib (name) {
  ...
  if (name in state.required) {
    ...
    state.required[name] = ''
    log.debug('↳ compiling')
    const old_log_status = show_log_messages
    try {
      show_log_messages = false
...
function tokenise (input, line) {
  ...
  if (/^\/\/[^/].*$/.test(input)) { // a comment begining with two "//"
    ...
    if (list.length >= 4) {
      const expr = tokenise(list.slice(3).join(' '), line) // extract all the letters after command
    ...
    if (list.length > 1) {
      const expr = tokenise(list.slice(1).join(' '), line) // extract all the letters after command
    ...
    if (list.length > 2) {
      const expr = tokenise(list.slice(2).join(' '), line) // extract all the letters after command
    ...
    if (list.length > 1) {
      const string_list = input.slice(3).split(';')
      const init = tokenise(string_list[0], line)
      const expr = tokenise(string_list[1], line)
      const cmd = tokenise(string_list[2], line)
    ...
    const matches = /^([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^=].*)$/.exec(input)
    const expr = tokenise(matches[2], line)
    ...
    let expr
    const expr_string = list.slice(1).join(' ')
    ...
    const expr = tokenise(list.slice(2).join(' '), line)
    ...
    const matches = /^\*(\(.*\))?([a-zA-Z_][a-zA-Z0-9_]*)$/.exec(input)
    let type_cast
    if (matches[1] !== undefined) {
      type_cast = matches[1].slice(1, -1)
    }
    const var_or_const_name = matches[2]
    ...
    const matches = /^([a-zA-Z_][a-zA-Z0-9_]*)\.(append|insert)\((.*)\)$/.exec(input)
    const array_name = matches[1]
    const operation = matches[2]
    const argument_string = matches[3]
    const args = argument_string.split(',')
    ...
    const matches = /^([a-zA-Z_][a-zA-Z0-9_]*)\[(.+)\]\s*=\s*(.*)$/.exec(input)
    const array_name = matches[1]
    const index_expression = matches[2]
    ...
    const matches = /^([a-zA-Z_][a-zA-Z0-9_]*)\.(len|pop|max_len)\(\)$/.exec(input)
    const array_name = matches[1]
    const operation = matches[2]
    ...
    const name = /^\S+?(?=\((.*)\)$)/.exec(input)[0]
    const args_string = /^\S+?(?=\((.*)\)$)/.exec(input)[1]
    const string_list = args_string.split(',')
    const args = []
    if (args_string !== '') {
    ...
    const operation = find_operation(/(>> 8)|(>>)|(<<)|(!=)|(<=)|(>=)|[\+\-\*\/\!\<\>\&\^\|\%:]|(==)|(\.\.)|(sys\.ov)|(sys\.odd)/g, input)
    if (operation in { '+': '', '-': '', '/': '', '*': '', '^': '', '%': '' }) { // dual operand [non bool]
      const args = input.split(operation)
      const expr1 = tokenise(args[0], line)
      const expr2 = tokenise(args[1], line)
    ...
    if (matches[2] !== undefined) {
      const type_size_string = matches[2].slice(1, -1)
      type_size = type_size_string.split(',')
    }

    const elements_string = matches[3].slice(1, -1)
    const elements_array = elements_string.split(',')
    const token_array = []

    for (const item of elements_array) {
  ...
  if (token.type === 'command') {
    ...
    switch (token.name) {
      case 'asm': {
        ...
        for (let i = 0; i < words.length; i++) {
          ...
          if (addr_operator || value_operator) {
            ...
            const [prefix, values] = translate(token)
      ...
      // arrays require a different allocation method
        ...
        const [expr_prefix, expr_value, expr_type] = translate(value, args.type)

        if (args.type !== expr_type) {
          throw new CompError(`Wrong data type, expected '${args.type}', got '${expr_type}'`)
        }

        const memory = alloc_block(expr_value.length)
        ...
        for (const expr_word of expr_value) {
      ...
      // arrays require a different allocation method
        ...
        const memory = alloc_global_block(expr_value.length)
      ...
      case 'arg_alloc': { // arg [type] [name] [expr]
        ...
        const label = `func_${state.scope}_${args.name}:`
        result.push(label)
      } break
      ...
      case 'var_array_alloc': {
        ...
        const [expr_prefix, expr_values, expr_type] = translate(args.expr)

        if (expr_type !== 'expr_array') {
          throw new CompError(`Array declarations require type 'expr_array', got '${expr_type}'`)
        }

        const [base_addr, current_len, max_len, data_type] = expr_values

        assert_valid_datatype(data_type)
        const element_size = data_type_size[data_type]

        const header_memory = alloc_block(3)
        const array_memory = alloc_block(max_len * element_size)
      ...
      case 'global_array_alloc': {
        ...
        const header_memory = alloc_global_block(3)
        const array_memory = alloc_global_block(max_len * element_size)
      ...
      case 'set': { // [name] = [expr]
        ...
        // this is a local variable or argument
          ...
          const table_entry = state.symbol_table[state.scope][args.name]

          if (!['variable', 'argument'].includes(table_entry.type)) {
            throw new CompError('Only variables and arguments can be modified')
          }

          const dst_regs = table_entry.specific.ram_addresses
          const dst_type = table_entry.data_type

          // get the value and type of the expression
          const [prefix, value, expr_type] = translate(args.expr, dst_type)
        ...
        // this is a global variable
          ...
          const table_entry = state.symbol_table['[global]'][args.name]
      ...
      case 'pointer_set': { //* [pointer] = [expr]
        const [expr_prefix, expr_value, expr_type] = translate(args.expr)
        const size = data_type_size[expr_type]

        // run code state.required by expression
        result = expr_prefix

        const ptr_expr = tokenise(args.name)
        const [ptr_prefix, ptr_value, ptr_type] = translate(ptr_expr)
      ...
      case 'array_set': {
        const array_name = args.name
        const index_expr = args.index_expr
        const expr = args.expr

        const [index_prefix, index_value, index_type] = translate(index_expr, 'int')
        if (index_type !== 'int') {
        ...
        const table_entry = state.symbol_table[scope_name][array_name].specific
        const array_type = table_entry.element_data_type

        const base_addr = `[${ram_prefix}${table_entry.base_addr}]`
        const item_size = data_type_size[array_type]
      ...
      case 'array_function': {
        ...
        const base_addr = `[${ram_prefix}${table_entry.base_addr}]`
        const current_len = `[${ram_prefix}${table_entry.current_len}]`
        const max_len = `[${ram_prefix}${table_entry.max_len}]`
        const item_size = data_type_size[array_type]
        ...
        // append just puts the given value at the current end of the list, then adds 1 to the size of the list
          ...
          const [index_expression, item_expression] = args.exprs

          // evaluate the expression that gives the index
          const [expr_prefix, expr_value, expr_type] = translate(index_expression, 'int')
          if (expr_type !== 'int') {
            throw new CompError("Array indexes must be of type 'int'")
          }
          result.push(...expr_prefix)

          const source_addr = get_temp_word()
          const target_addr = get_temp_word()
      ...
      case 'return': { // return [optional expr]
        ...
        const func_type = state.function_table[state.scope].data_type
        const force_cast = state.function_table[state.scope].force_cast
        ...
        if (func_type !== 'none') {
          const [prefix, value, expr_type] = translate(args.expr, func_type)
          ...
          for (const item of value) {
            const temp = item.replace('ram', 'ram+')
    ...
    // number types
      case 'number': { // a generic number that can be turned into the type state.required by the context
        ...
        const num_token = { name: type, type: 'expression', arguments: { value: args.value } }
        const prefix_register_type = translate(num_token, type)
      ...
      case 'int': {
        ...
        const dec_val = parse_int(args.value)
      ...
      case 'long': {
        ...
        const bin = pad(dec_val.toString(2), 32)
        const high = `0b${bin.substring(0, 16)}`
        const low = `0b${bin.substring(16, 32)}`
      ...
      case 'str': {
        ...
        const string = args.value.slice(1, -1)

        const id = `str_${gen_id('str')}`
      ...
      case '>=': {
        ...
        switch (ctx_type) {
          case 'int': {
            const temp_vars = [get_temp_word(), get_temp_word()]
      ...
      case '..': {
        log.debug(`op: ${token.name}, target type: ${ctx_type}`)
        const expr1 = translate(args.expr1, ctx_type)
        const expr1_prefix = expr1[0]
        const expr1_reg = expr1[1]
        prefix = expr1_prefix
        const expr2 = translate(args.expr2, ctx_type)
        const expr2_prefix = expr2[0]
        const expr2_reg = expr2[1]
      ...
      case ':': { // word selector
        log.debug(`op: ${token.name}, target type: ${ctx_type}`)
        const expr = translate(args.expr1, ctx_type)
        prefix = expr[0]
        const expr_regs = expr[1]

        const index = translate(args.expr2, 'int')
        if (index[2] != 'int') {
      ...
      case 'is_odd': {
        log.debug(`op: ${token.name}, target type: ${ctx_type}`)
        const prefix_and_value = translate(args.expr, 'int')
        prefix = prefix_and_value[0]
      ...
      case 'expr_array': {
        const label = `expr_array_${gen_id('expr_array')}`

        const given_type_size = args.type_size
        const length = args.exprs.length
        ...
        const item_size = data_type_size[contained_type]
        const consts_to_add = [`${label}:`]

        let prefix_and_value
        for (const item of args.exprs) {
      ...
      case 'var_or_const': {
        ...
        // it's a regular variable/argument
          for (const addr of table_entry.specific.ram_addresses) {
            if (is_global) {
        ...
        // it's a constant
          for (const addr of table_entry.specific.value) {
            registers.push(`[${addr}]`)
          }
        } else {
      ...
      case 'array_expression': {
        ...
        if (operation === 'index') {
          const prefix_value_type = translate(args.expr, 'int')
          if (prefix_value_type[2] != 'int') {
            throw new CompError("Array indexes must be of type 'int'")
          }
          prefix.push(...prefix_value_type[0])

          const index = prefix_value_type[1]
      ...
      // functions begining with 'sys.' will need to be loaded from the standard library first
        ...
        const function_table_entry = state.function_table[args.name]

        const target_args = function_table_entry.arguments
        const target_arg_no = target_args.length
        const actual_arg_no = Object.keys(args.exprs).length
        ...
        for (let i = 0; i < actual_arg_no; i++) {
          const target_type = state.symbol_table[args.name][target_args[i]].data_type
          const target_regs = state.symbol_table[args.name][target_args[i]].specific.ram_addresses

          const expr_token = args.exprs[i]

          const [expr_prefix, expr_value, expr_type] = translate(expr_token, target_type)
          ...
          for (let y = 0; y < target_regs.length; y++) {
            const expr_word = expr_value[y]
            const arg_word = `ram+.${target_regs[y]}`
      ...
      case 'pointer_lookup': {
        const address_expr = tokenise(args.var_or_const_name)
        const [addr_prefix, addr_value, addr_type] = translate(address_expr, 'int')
        if (addr_type !== 'int') {
        ...
        const size = data_type_size[type]
        const temp_buffer = alloc_block(size)

        // make pointer value available
        prefix.push(...addr_prefix)

        // copy pointer value into temp ram word
        const pointer_addr = get_temp_word()
    ...
    switch (token.name) {
      case 'if': {
        ...
        for (let i = 0; i < exprs.length; i++) {
          ...
          const prefix_and_value = translate(exprs[i], 'bool')
          const prefix = prefix_and_value[0]
          const value = prefix_and_value[1]
      ...
      case 'for': {
        const label = `for_${gen_id('for')}`
        const init_result = translate(args.init)
        result.push(...init_result)

        const expr_prefix_and_value = translate(args.expr, 'bool')
        const expr_prefix = expr_prefix_and_value[0]
        const expr_value = expr_prefix_and_value[1][0]
        ...
        const prev_state = state.inner_structure_label
        ...
        const cmd_result = translate(args.cmd)
      ...
      case 'while': {
        const label = `while_${gen_id('while')}`

        const prefix_and_value = translate(args.expr, 'bool')
        const prefix = prefix_and_value[0]
        const value = prefix_and_value[1][0]
        if (prefix_and_value[2] != 'bool') {
          throw new CompError(`Conditional expression expected type 'bool', got '${prefix_and_value[2]}'`)
        }
        result.push(...prefix)
      ...
      case 'repeat': {
        ...
        const body = translate_body(token.body)
        prefix = []
      ...
      case 'function_def': {
        ...
        const label = `func_${args.name}:`
        state.funcs[args.name] = [label]
        const target = state.funcs[args.name]
...
function compile (input, nested) {
  ...
  for (let i = 0; i < input.length; i++) {
    const line = input[i].trim() // remove any trailing whitesapce
  ...
  // add function defs
  for (const item in state.funcs) {
    for (const line of state.funcs[item]) {
      output += '\n' + line
    }
  }
  ...
  if (!nested) {
    ...
    for (const namespace in state.symbol_table) {
      if (namespace.startsWith('sys.')) {
        continue
      }
      const symbols = state.symbol_table[namespace]
    ...
    const ram_percent = Math.round((state.max_allocated_ram_slots / 1023) * 100)
